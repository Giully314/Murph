...
export module murph.math.mat4d;

import std;

import murph.types;
import murph.math.concepts;
import murph.math.vec4d;
...
public:
	...
	[[nodiscard]]
	constexpr auto&& operator[](this Self&& self, const u32 i, const u32 j) {
		return std::forward<Self>(self).m[i][j];
	}

	template <typename Self>
	[[nodiscard]]
	constexpr auto&& operator[](this Self&& self, const u32 i) {
	...
	constexpr auto Determinant() const noexcept -> T {
		T determinant{ 0 };
		const T s00 = m[2][2] * m[3][3] - m[2][3] * m[3][2];
		const T s01 = m[2][1] * m[3][3] - m[2][3] * m[3][1];
		const T s02 = m[2][1] * m[3][2] - m[2][2] * m[3][1];
		const T s03 = m[2][0] * m[3][3] - m[2][3] * m[3][0];
		const T s04 = m[2][0] * m[3][2] - m[2][2] * m[3][0];
		const T s05 = m[2][0] * m[3][1] - m[2][1] * m[3][0];
	...
	constexpr auto Inverse() const -> Mat4d {
		...
		const T s04 = m[2][0] * m[3][2] - m[2][2] * m[3][0];
		const T s05 = m[2][0] * m[3][1] - m[2][1] * m[3][0];
		const T s06 = m[1][2] * m[3][3] - m[1][3] * m[3][2];
		const T s07 = m[1][1] * m[3][3] - m[1][3] * m[3][1];
		const T s08 = m[1][1] * m[3][2] - m[1][2] * m[3][1];
		const T s09 = m[1][0] * m[3][3] - m[1][3] * m[3][0];
		const T s10 = m[1][0] * m[3][2] - m[1][2] * m[3][0];
		const T s11 = m[1][0] * m[3][1] - m[1][1] * m[3][0];
		const T s12 = m[1][2] * m[2][3] - m[1][3] * m[2][2];
		const T s13 = m[1][1] * m[2][3] - m[1][3] * m[2][1];
		const T s14 = m[1][1] * m[2][2] - m[1][2] * m[2][1];
		const T s15 = m[1][0] * m[2][3] - m[1][3] * m[2][0];
		const T s16 = m[1][0] * m[2][2] - m[1][2] * m[2][0];
		const T s17 = m[1][0] * m[2][1] - m[1][1] * m[2][0];
